...
public class RoadFollower {
    ...
    // 左侧道路跟随参数 - 增加距离让机器人离左侧边缘更远
    private float mLeftLaneDistance = 220f;    // 与左侧道路的理想距离（像素）
    private float mDistanceTolerance = 40f;    // 距离容差
    ...
    // 设置与左侧道路的距离 - 扩大可设置范围
    public void setLeftLaneDistance(float distance) {
        mLeftLaneDistance = Math.max(200f, Math.min(500f, distance)); // 范围 200-500
    }
    ...
    private void controlMovement(Mat lines, Mat debugFrame) {
        List<LineSegment> leftLaneCandidates = new ArrayList<>();
        ...
        for (int i = 0; i < lines.rows(); i++) {
            ...
            // 只关注左侧车道线（负斜率）且过滤接近水平的线
            if (slope < -0.3 && slope > -5.0) {
                leftLaneCandidates.add(new LineSegment(x1, y1, x2, y2, slope));
            }
        }

        if (leftLaneCandidates.isEmpty()) {
            // 没有检测到左侧道路，缓慢前进并轻微向右偏移（因为要离左侧边缘更远）
            SimpleMoveWrap.setLinearVelocity(mMinLinearVelocity * mSpeedMultiplier);
            SimpleMoveWrap.setAngularVelocity(-0.05f); // 轻微向右转以远离左侧边缘
            return;
        }

        // 找到最靠近中心线的左侧道路边线
        LineSegment closestLeftLane = findClosestLeftLane(leftLaneCandidates, frameCenter, frameBottom);

        if (closestLeftLane != null) {
            // 计算与左侧道路的交互点（在图像底部的位置）
            double leftLaneAtBottom = closestLeftLane.getXAtY(frameBottom);

            // 计算目标位置：左侧道路位置加上理想距离（更大的距离）
            double targetPosition = leftLaneAtBottom + mLeftLaneDistance;
    ...
    /**
     * 找到最靠近中心线的左侧道路边线
     * @param candidates 候选的左侧车道线列表
     * @param frameCenter 图像中心x坐标
     * @param frameBottom 图像底部y坐标
     * @return 最靠近中心线的左侧车道线
     */
    private LineSegment findClosestLeftLane(List<LineSegment> candidates, int frameCenter, int frameBottom) {
    ...
    private void drawDetectionResult(Mat frame, Mat lines, Mat edges) {
        ...
        // 绘制所有检测到的左侧直线（浅绿色）
        for (LineSegment lane : leftLaneCandidates) {
            Imgproc.line(frame,
        ...
        // 找到并突出显示最靠近中心线的左侧道路边线（红色）
        LineSegment closestLane = findClosestLeftLane(leftLaneCandidates, frameCenter, frameBottom);
        if (closestLane != null) {
            Imgproc.line(frame,
            ...
            // 绘制目标位置（机器人应该到达的位置）
            double targetX = bottomX + mLeftLaneDistance;
        ...
        // 绘制理想距离线（黄色）- 现在是从左侧车道算过来的
        int targetLineX = frameCenter - (int)mLeftLaneDistance;
        Imgproc.line(frame,
        ...
        // 添加距离信息文本
        String distanceText = "Left Distance: " + (int)mLeftLaneDistance + "px";
    ...
    public float getLeftLaneDistance() {
        return mLeftLaneDistance;
    }

    // 新增方法：直接设置更大的距离（左侧）
    public void setSafeDistance() {
        mLeftLaneDistance = 400f; // 设置一个更安全的距离
    }
}
